<template>
    <section v-if="stay" class="stay-details main-layout">
        ...
        <section class="stay-display-divider flex gray-box-shadow" ref="displayContainer">
            <div class="stay-display-info">
                <section class="stay-summery-container gray-box-shadow flex space-between align-center">
                    <div>
                        <h2 class="stay-summery">{{ stay.summary }} hosted by {{ hostName }}</h2>
                        <h5 class="stay-capacity">{{ stay.capacity }} guests · 3 bedrooms · 3 beds · 2 baths</h5>
                    </div>
...
export default {
    ...
    computed: {
        ...
        reviewsAvg() {
            const sum = this.stay.reviews.reduce((sum, review) => sum + review.rate, 0);
