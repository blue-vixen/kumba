<template>
    <div v-if="alive" class="alert gray-box-shadow flex space-around" :class="{ on: alive }">
        ...
        <div class="msg-btns">
            ...
            <button @click="loginDemoUser" class="clean-btn clickable nice-btn">Demo User</button>
            <button @click="closeIt" class="clean-btn clickable close-modal"><span class="material-icons"> close </span></button>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async loginDemoUser() {
            ...
            try {
                await this.$store.dispatch({ type: "login", userCred: demoUser });
            } catch (err) {
                console.log(err);
